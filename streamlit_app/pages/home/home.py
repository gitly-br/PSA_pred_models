...
with col1:
    ...
    if st.session_state.data:
        ...
        for json_response in st.session_state.data['data']:
            data_list_summary[json_response['regiao']] = json_response.get('summary', None)
            data_list_detailed[json_response['regiao']] = json_response.get('detailed',None)

flag_not_rain_predict =  data_list_summary['SA'] is not None

if flag_not_rain_predict:
    st.session_state.rain_distribution = get_rain_distribution(data_list_summary)
    proba_rain_distribution = get_color_distribution(data_list_summary['SA'].get('proba', 0), st.session_state.rain_distribution)
...
with col2:
    with st.container(border=True):
        ...
        st.markdown(
        f"""
        <div style='background-color: rgba{str(get_color(data_list_summary['SA']['proba'], 0.3) if flag_not_rain_predict else (171, 171, 171, 0.45))}; display: flex; flex-direction: column; align-items: center; justify-content: center; margin-bottom: 10px; padding: 10px; border-radius: 10px;'>
...
with col_2:
    ...
    with a1:
        with st.container(border=True):
            plot_gauge(data_list_summary['TAMCENTRAL']['proba'] if flag_not_rain_predict else 0, "Bacia do Tamanduateí Central", "TAMCENTRAL", {'l':10, 'b':20, 't':50}, flag_not_rain_predict)
    with a2:
        with st.container(border=True):
