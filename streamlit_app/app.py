...
if not st.session_state.logged_in:
    ...
    if response.status_code == 200:
        ...
        if json_response.get('status') == 'success':
            ...
            df.rename(columns={'region': 'regiao', 'score': 'valor'}, inplace=True)
            ...
            # Plota o mapa
            st.map(df, latitude="lat", longitude="lon", size="circle_rad", color="cor")

            # Exibe a lista ao lado direito
            st.sidebar.title("Regiões")
            for idx, row in df.iterrows():
                st.sidebar.markdown(f"**{row['regiao']}**")
                st.sidebar.markdown(
                    f"<div style='background-color:{row['cor']};width:20px;height:20px;border-radius:50%;'></div>",
