...
def fit_avaliar(mk, sw, X_tr, y_tr, X_te, y_te, datas_te, w_tr):
    thr = _thr_cv_treino(mk, sw, X_tr, y_tr, w_tr)
    clf = mk(sw)
    sw_arr = np.where(y_tr == 1, w_tr, 1.0).astype(float)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        _fit_with_sw(clf, X_tr, y_tr, sw_arr)
...
for bacia in sorted(df_ml["bacia"].unique().to_list()):
    ...
    pos_cham_tr = train["pos_chamado"].cast(pl.Int8).to_numpy()
    # peso por positivo: 1.0 se chamado confirmado; 0.5 se positivo só por fonte externa
    w_tr = np.where(pos_cham_tr == 1, 1.0, 0.5)
    datas_te = test["data"]
    if y_te.sum() == 0:
        continue
    sw = float((y_tr == 0).sum()) / max((y_tr == 1).sum(), 1)
    for nome, mk in MODELOS.items():
        res = fit_avaliar(mk, sw, X_tr, y_tr, X_te, y_te, datas_te, w_tr)
