"""
Benchmark V8 — Leak-corrected, com holdout dedicado.

Leak corrigido: api_* usa shift(1) em max_dia (mesmo do export_champion.py).
Holdout: threshold é tunado em 50% do treino via TimeSeriesSplit,
         modelo final é avaliado nos 50% restantes do treino (val_holdout),
         e só então predict no test set.
"""
...
def avaliar_bacia(bacia, sub, modelos_dict):
    t_cut_local = T_CUT
    # oratorio usa cut próprio
    if bacia == "oratorio":
    ...
    # Holdout: última quarter do treino = val_holdout
    ...
    results = {}
    for nome, mk_clf in modelos_dict.items():
        ...
        for k in [1, 2, 3]:
            ...
            # threshold tunado no val_holdout (TimeSeriesSplit só no train portion)
            ...
            # modelo final treinado em TODO train (75% dos dados)
            clf = mk_clf(0.0)
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
        ...
        # alarme nível = maior k com prob >= thr
        ...
        # métricas no val_holdout (threshold tuning set)
        y_bin_val_m1 = (sev_val >= 1).astype(int)
        prob_m1_val = probs_val[1]
        for k in [1, 2, 3]:
        ...
        # ── Métrica de monotonicidade: Spearman entre max_dia e prob ≥1 ──
        # usa val_holdout (dados não vistos durante tuning)
        max_dia_val = val_holdout["max_dia"].to_numpy()
        prob_evento_val = probs_val[1]  # P(severidade >= 1)
        from scipy.stats import spearmanr
        corr, pval = spearmanr(max_dia_val, prob_evento_val)
...
def main():
    ...
    for bacia in sorted(estacoes_bacia.keys()):
        ...
        r = avaliar_bacia(bacia, sub, modelos)
        if r is None:
        ...
        print(f"\n  Validação holdout (75% train → 25% val):")
        print(f"  {'Modelo':<12}  {'k':>2}  {'Thr':>6}  {'PRAUC':>6}  {'Recall':>7}  {'Prec':>6}  {'F1':>5}  {'Spearman ρ':>10}")
        for nome, row in r.items():
            for k in [1, 2, 3]:
                print(f"  {nome:<12}  {k:>2}  {row[f'thr_k{k}']:>6.3f}  "
                      f"{row[f'prauc_k{k}']:>6.3f}  {row[f'recall_k{k}']:>7.3f}  "
                      f"{row[f'prec_k{k}']:>6.3f}  {row[f'f1_k{k}']:>5.3f}  "
                      f"{row['spearman_rho']:>10.3f}")
        print(f"\n  Test set (holdout não visto):")
        print(f"  {'Modelo':<12}  {'k':>2}  {'PRAUC':>6}  {'Recall':>7}  {'Prec':>6}  {'F1':>5}")
        for nome, row in r.items():
            for k in [1, 2, 3]:
                print(f"  {nome:<12}  {k:>2}  "
                      f"{row[f'prauc_test_k{k}']:>6.3f}  {row[f'recall_test_k{k}']:>7.3f}  "
                      f"{row[f'prec_test_k{k}']:>6.3f}  {row[f'f1_test_k{k}']:>5.3f}")
    ...
    out_path = OUT_DIR / "benchmark_v8_leak_corrected.parquet"
